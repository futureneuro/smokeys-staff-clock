-- Per-assignment break allowance for each staff shift

alter table if exists public.shift_assignments
    add column if not exists break_minutes_allowed integer not null default 60;
...
begin
    if not exists (
        select 1
        from pg_constraint
        where conname = 'shift_assignments_break_minutes_allowed_chk'
    ) then
        alter table public.shift_assignments
            add constraint shift_assignments_break_minutes_allowed_chk
                check (break_minutes_allowed >= 0);
    end if;
end $$;
